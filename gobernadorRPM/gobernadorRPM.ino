...
Screen screen1 = Screen(0x27);
MonitorRpm monitorRpm = MonitorRpm();
...
unsigned long TCALOR;//variable que almacena el tiempo en millis necesaria para evitar los rebotes de activado del PIN_VENTILADOR, si alcanza una alara de temperatura el PIN_VENTILADOR se activa durante 5 segudos y luego se apaga, esto porque si no el PIN_VENTILADOR se prendia y se apagaba a cada ratito
unsigned long TCALOR2;//variable que almacena el tiempo en millis necesaria para evitar los rebotes al cortar la PIN_BOMBA por sobre temperatura
...
bool temperaturaEstimadaLlegoMaximo = false; //para saber si la mnEprom.settings.stimateTemperature llego a su maximo valor, y entonces ahora asignarle el valor real de la temperatura y no seguir aumentando 10 grados por cada minuto encendido,
...
//Añadimos el puente del PIN_VENTILADOR, dejando presionada la tecla de arriba por 10 segundos, hace que cada que el swich se active el PIN_VENTILADOR se encienda y quede asi siempre
...
//las mostramos con el motor encendido ignicion 0 y apagado pero con el swich en off al presionar el boton salir como Ts (mnEprom.settings.stimateTemperature) y como E (estado de alarmas temperatura 0 activas 1 inactivas)
...
//se Añade la alarma del puente del PIN_VENTILADOR, porque el puente del PIN_VENTILADOR a veces los agentes lo activaban sin darce cuenta, y se quedaba asi por mucho tiempo
//hasta que el arnes se derretia, la alarma suena cuando se abre el swich, y cada 3 minutos que el motor va encendido.
//Se evita que las alarmas de swich avierto y puente PIN_VENTILADOR se ensimen.
...
void setup() {
    ...
    pinMode(PIN_IMPRESORA, OUTPUT);
    pinMode(PIN_BLOQUEO, INPUT_PULLUP);             //CONECCION CORTE GPS CON EL PULLUP SIMPLEMENTE EL PIN SIEMPRE MANDA 1 CUANOD EL GPS NO ESTA A TIERRA O ESTA DESBLOQUEADO  Y CUANDO EL GPS MANDA A TIERRA SE PONE A 0
    pinMode(PIN_CAJA, OUTPUT);
    pinMode(PIN_VENTILADOR, OUTPUT);
    pinMode(PIN_WDT, OUTPUT);
    
    pinMode(PIN_GLP, INPUT_PULLUP);                 //PIN QUE RECIBE DEL TRANSISTOR 1 CUANDO ESTA EN MODO GASOLINA Y 0 CUANDO ESTA EN MODO GAS, CUANDO LA ELECTROVALVULA LE MANDA 12V A LA BASE DEL TRANSISTOR SE PONE EN 0 EL COLECTOR  
    pinMode(PIN_SWITCH, INPUT_PULLUP);              // lectura de la posicion del swich
    pinMode(PIN_LUCES, INPUT_PULLUP);
    pinMode(PIN_BUZZER, OUTPUT);
    pinMode(PIN_BOMBA, OUTPUT);
    pinMode(PIN_AHORRO_ENERGIA, OUTPUT);
    ExternalWDT::reset();
    digitalWrite(PIN_AHORRO_ENERGIA, HIGH);         //activamos inmediatamente el mosfet que d ala energia al sistema
    digitalWrite(PIN_BOMBA, HIGH);                  //activamos la PIN_BOMBA de gasolina
    digitalWrite(PIN_BUZZER, HIGH);delay(20);digitalWrite(PIN_BUZZER, LOW);
    digitalWrite(PIN_VENTILADOR, HIGH);
    delay(50);
    digitalWrite(PIN_VENTILADOR, LOW);
    digitalWrite(PIN_IMPRESORA, LOW);
    digitalWrite(PIN_CAJA, LOW);
...
void loop() {
    ...
    swich = digitalRead(PIN_SWITCH);
    gps = digitalRead(PIN_BLOQUEO);
    ...
    //APAGADO POR AHORRO DE ENERGIA.
       ...
       //energia tanto al GPS como al arduino y el sistema completo. El GPS consume entre 120 a 190 ma y ardiono con la luz de fondo del screen1 consume 48ma
    ...
    if (ignicion == 0 || gps == 0) {
        ...
        if (mnEprom.settings.contPowerSave > 2) {
            mnEprom.settings.contPowerSave = 0;
            EEPROM.put(21, mnEprom.settings.contPowerSave);
        }
    }
    ...
    if (millis() > tiempoAhorroEnergia + 1800000) {
        ...
        mnEprom.settings.contPowerSave++; //aumentamos 1 al valor del contador
        ...
        if (mnEprom.settings.contPowerSave >= mnEprom.settings.thresholdPowerSave) {
            mnEprom.settings.contPowerSave = 0;
            EEPROM.put(21, mnEprom.settings.contPowerSave);
            digitalWrite(PIN_AHORRO_ENERGIA, LOW); //APAGAMOS EL MOSFET, EN ESTE MOMENTO EL SISTEMA SE APAGA COMPLETAMENTE GPS Y GOBERNADOR OBTENEMOS 0 CONSUMO DE ENERGIA

        }
        else {
            EEPROM.put(21, mnEprom.settings.contPowerSave); //guardamos el nuevo valor del contador en la eeprom
            tiempoAhorroEnergia = millis();
        }
    }
    ...
    if (mnEprom.settings.stimateTemperature > 200) {
        mnEprom.settings.stimateTemperature = 0;
        EEPROM.put(28, mnEprom.settings.stimateTemperature);
    }
    ...
    if (ignicion == 0 && millis() > tiempoDeMotorEncendidoTemperaturaEstimada + 60000) {
        ...
        if (!temperaturaEstimadaLlegoMaximo) {
            ...
            mnEprom.settings.stimateTemperature += 10;

            if (mnEprom.settings.stimateTemperature >= 100) {
                mnEprom.settings.stimateTemperature = 100;
            ...
            if (temperaturaD2 <= mnEprom.settings.stimateTemperature - 7 || temperaturaD2 >= mnEprom.settings.stimateTemperature + 7) {
                mnEprom.settings.stimateTemperature = byte(temperaturaD2);//le metemos a un byte un float a ver si no hay algun error
                EEPROM.put(28, mnEprom.settings.stimateTemperature);
            }
            ...
            /*supongamos que se enciende el motor, pasan 10 minutos y la temperatura estimada alcanza su maximo de 100 grados, supongamos que el motor llego a 85 grados reales
              ...
              margen de temperatura menor porque por ejemplo cuando el motor va funcionando la temperatura esta entre 98, baja 1 grado o sube 1 o baja 3 por el PIN_VENTILADOR etc
    ...
    if (ignicion == 0 && mnEprom.settings.stimateTemperature >= 75) {
 ...
 //calculo de rpm solo si ya pasaro 250ms
     //segun mi hoja de excel "Calculo de inyeccion" la PIN_RPM a 1000rpm se activaria en 1 segundo 8.3 veces como hay 2 pulsos en el tiempo de 1 serian 16 veces por segundo
     ...
     //digital invertida a arduino. la entrada del arduino siempre va a estar en 5v alto, cuando haya un pulso en la PIN_RPM es decir hay un 1 en la base del transistor
...
/* if (millis () > millisRpm + 500){
   ...
   attachInterrupt(digitalPinToInterrupt(PIN_RPM), funcionInterrupcion, CHANGE);
 }
 */
 ...
 //CORTE POR REVOLUCIONES
    ...
    if (rpm >= mnEprom.settings.thresholdRpm) {
        ...
        if (!rpmParasita) {
            if (rpm >= mnEprom.settings.thresholdRpm - 250) {
                digitalWrite(PIN_BUZZER, HIGH);
            }
            else {
   ...
   //Promediado de las lecturas de los sensores y niveles de voltaje analogicos
    ...
    if (millis() > tiempopromedio + 1000) //leemos cada segundo las lecturas analogicas
    {
        ...
        //LUCES //si el motor se apaga y las luces siguen encendidas
        luces = digitalRead(PIN_LUCES);
        if (swich == 1 && luces == 0) {
            Aluces = 1;
            A = 1;
        }
        ...
        {
            sumaTemperatura += (1023 - analogRead(PIN_ANG_TEMPERATURE)) / mnEprom.settings.adjustTemp;
            bgDSuma += analogRead(PIN_ANG_BATTERY) / mnEprom.settings.adjustVoltaje;
    ...
    if (temperaturaD2 >= mnEprom.settings.thresholdAlmTemp && temperaturaD2 < mnEprom.settings.thresholdCutTemp && activartemperatura == 0) {
        alarmaT = 1;A = 1;
        digitalWrite(PIN_VENTILADOR, HIGH);

        TCALOR = millis();
    }
    //si la temperatura es mayor al limitede alarma y menor al limite de corte, y transcurrio el tiempo para activartempertatura , se activa la alarma de temperatura, se enciende el PIN_VENTILADOR y se guarda el tiempo de activado

    if (temperaturaD2 < mnEprom.settings.thresholdAlmTemp || activartemperatura == 1) {
        alarmaT = 0;
    ...
    //si la temperatura es menor al limite de alarma y han transcurrido 60s desde que se activo la alarma y durante el cual el PIN_VENTILADOR esta encendido entonces se apaga la alarma
    //y el PIN_VENTILADOR, primero apagamos la alarma, el PIN_VENTILADOR se quedara encendido 60 segundos

    if (temperaturaD2 >= mnEprom.settings.thresholdCutTemp && activartemperatura == 0) { corteT = 1;A = 1;TCALOR2 = millis(); }
    ...
    if (temperaturaD2<mnEprom.settings.thresholdCutTemp && activartemperatura == 0 && millis()>TCALOR2 + 8000) { corteT = 0; }
    //cuando la temperatura decrese del limite de corte  espera 8 segundos para desactivar la alarma y volver a activar la PIN_BOMBA de gasolina,
    ...
    //la boba se apagaba y se encendia, un rebote, con este retardo nos aseguramos que en cuanto la alarma detone se apage la PIN_BOMBA  

    if (activartemperatura == 1) { corteT = 0; }
    //para que cuando se apage el motor o el swich deje de sonar la alarma y active denuevo la PIN_BOMBA
    ...
    if (mnEprom.settings.almBatery) {//si las alarmas se activan en el menu
    ...
    //ACTIVACION Y CANCELACION DEL MOTOR // si el swich esta en posicion de encendido el gps esta desbloqueado y no esta activo el corte por rpm ni el dorte por temperatura entonces encendemos la PIN_BOMBA y apagamos el led
    if (rele == 0 && gps == 1 && corteT == 0 && maxrpm == 0) {
    ...
    if (swich == 1 && millis() > Trele + 15000) { rele = 1; }                     // si swich de gira a posicion de apagado, se tendra que esperar 15 segundos para que rele cambie a 1 y haga que ya no se cumpla la condicion de activacion del motor, lo cual hara que el relevador que activa la PIN_RPM se apage


  /*se agrega estas lineas porque el rle usado en el corte de la PIN_RPM siempre se queda energisado aunque el motor esta apagado. estamos usando los contacots NA 30 y 87
   * es decir el motor solo encendera hasta que se energise el rele y esto es enviando un estado HIGH al pin PIN_BOMBA, el problema venia que al apagar el motor el rele se
   ...
   * hacemos que el if de activacion de la PIN_BOMBA se valide y por lo tanto el rele se energise y cierre sus contactos permitiendo que el motor encienda
   ...
   //PANTALLA
    if (millis() > pantalla1 + 125) {//frecuencia de muestreo
     //PANTALLA GENERAL____________________________________________________________________
        ...
        monitorRpm.stop();
        //rpm = numInterrupt * 125; //cuando se cuentan 8 pulsos cada 250 milis, es decir cuando se toma la señal de ambos cables de la PIN_RPM con 1 diodo
        ...
        //el PIN_BUZZER suena y a veces apaga el motor, esto porque es como si entraran señales paracitas y deprotno el sistema marca 3200rpm
        ...
        if (G == 1 && BOTONES() == 0) {// cuando G sea igual a 1 va a entrar a la muestra de pantalla general
            ...
            screen1.mainInfo(rpm, bgD, temperaturaD2, Minutos, Segundos, modo,false);
        ...
        if (A == 1 && millis() > alarmas1 + 6000) {//si se activo alguna alarma entra a la pantalla de alarmas se queda 2 segudnos en el mensaje, despues va a pantalla general hasta que millis sea mayor a 5000-2000=3000
            ...
            if (alarmaT == 1 && corteT == 0) {
                screen1.almTemperature(String(temperaturaD2));
                Audio::alarma();
            }

            if (corteT == 1) {
                screen1.almTemperatureOut(String(temperaturaD2));                
                Audio::grave();
            }
            ...
            if (Abg == 1) {
                //bgD
                screen1.almFallaAlternador(String(bgD));
                Audio::grave();;
            }



            if (Abg == 2 && Aswich == false) {
                screen1.almLowBattery();
                Audio::alarma();
                conteobateriabaja++;
            ...
            if (Aswich == 1) {
                //para evitar que se encimen las alarmas de swich abierto y del puente de PIN_VENTILADOR de esta manera le damos prioridad a la del puente
       ...
       //pantalla de Configuraciones__________________________________si precionamos el boton arriba nos muestra esta pantalla
        if (BOTONES() == BOTON_3_ARRIBA) {
            screen1.info1(rpm, mnEprom.settings.thresholdAlmTemp, mnEprom.settings.thresholdCutTemp);           
            
        }
        if (BOTONES() == BOTON_2_ABAJO) {
            screen1.info3(String(digitalRead(PIN_BOMBA)), String(ignicion), String(luces), String(swich),
                            String(digitalRead(PIN_VENTILADOR)), String(digitalRead(PIN_GLP)), String(gps));        

        }
        if (BOTONES() == BOTON_4_SALIR) {
            screen1.info2("","", String(mnEprom.settings.stimateTemperature), String(activartemperatura), VERSION_SOFTEARE);
    ...
    if (conteoReiniciarTemperaturaEstimada >= 10) { //cuando el conteo del menu llege a esta cifra ingresamos al menu
        ...
        screen1.print("Temp estimada");
        screen1.setCursor(0, 1);
        screen1.print("Reiniciada a " + String(mnEprom.settings.stimateTemperature));
        Audio::exito();
        delay(1000);
    ...
    if (swich == 0) {//MIENTRAS EL MOTOR ESTA ENCENDIDO
        ...
        if (estadopantalla == 0 && millis() > tiempolucesfinal + tiempoluces) {//si elswich esta en posicion de apagado sige con el programa hasta que el tiempo del programa sea mayor a el ultimo tiempo tomado cuando el motor estaba encendido y le suma el tiempo que quiera el usuario 
            screen1.setBacklight(LOW); //una vez se cumple el tiempo programado se apaga la luz
            estadopantalla = 1; // el estado de la pantalla ahora esta apagado
        }
...
bool ingresaAlMenu() {
    ...
    screen1.clear();
    screen1.print("INGRESE COD: " + randNumber);
    ...
    while (true) {
        ...
        if (millis() > retardo + 400) {
            ...
            case BOTON_1_ACEPTAR:
                ...
                codigoIngresado += "1";
                retardo = millis();
                screen1.setCursor(0, 0);screen1.print("COD:" + String(randNumber) + " " + String(tiemporegresivo) + "seg.");
                screen1.setCursor(0, 1);screen1.print(codigoIngresado);
                Audio::beep();
                break;
        ...
        if (tiemporegresivo == 0) {
            if (codigoIngresado == "42341432") {
                screen1.clear();
                screen1.print("CODIGO CORRECTO");
                screen1.setCursor(0, 1);
                screen1.print("BIENVENIDO");
                Audio::exito();
                ExternalWDT::reset();
                delay(1000);
                return true;
                ...
                screen1.clear();
                screen1.setCursor(0, 1);screen1.print("Abriendo Caja");
                digitalWrite(PIN_CAJA, HIGH);
                Audio::exito();
                ExternalWDT::reset();
                delay(700);
                ExternalWDT::reset();
                ...
                screen1.clear();
                screen1.print("El codigo es");
                screen1.setCursor(0, 1);
                screen1.print("incorrecto");
                Audio::error();
                ExternalWDT::reset();
                delay(800);
                return false;
...
bool menuPrincipal() {
    ...
    while (true) {//entramos a un bucle infinito
       ...
       // AUMENTAR LA VARIABLE LUGAR DEPENDIENDO DEL BOTON
        ...
        if (millis() > retardoBotones + 250) {//retardo entre clics
            ...
            case BOTON_4_SALIR:
                ...
                Audio::beep();
                screen1.clear();
                screen1.print("SALIENDO DE MENU");
                delay(2000);
                return false;
        ...
        case 1:
            ...
            while (submenu == 1) {//entramos al submenu solo si se preciona el boton aceptar y estamos afuera de un submenu     
                ...
                if (BOTONES() == BOTON_3_ARRIBA) { //si funcion botones retorna 50 es decir que se presion el boton arriba
                    if (millis() > retardo + 400) {
                        mnEprom.settings.thresholdRpm += 125;
                        retardo = millis();
                        Audio::beep();
                ...
                if (mnEprom.settings.thresholdRpm < 2000) {
                    mnEprom.settings.thresholdRpm = 2000;
                    Audio::error();
                }

                if (mnEprom.settings.thresholdRpm > 5000) {
                    mnEprom.settings.thresholdRpm = 5000;
                    Audio::error();
                }
                ...
                if (BOTONES() == BOTON_1_ACEPTAR) { // si se presiona aceptar
                    if (millis() > retardo + 400) {
                        retardo = millis();
                        submenu = 0;
                        EEPROM.put(0, mnEprom.settings.thresholdRpm);
                        Audio::guardar();
                ...
                if (BOTONES() == BOTON_4_SALIR) { // si se presiona salir
                    if (millis() > retardo + 400) {
                        submenu = 0;
                        retardo = millis();
                        EEPROM.get(0, mnEprom.settings.thresholdRpm);
                        Audio::beep();
                        screen1.clear();
                        screen1.print("SALIENDO");
                        delay(500);
                ...
                if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del meno de opciones 
                    ...
                    ExternalWDT::reset();
                    screen1.clear();
                    screen1.print("LIMITE: " + String(mnEprom.settings.thresholdRpm) + "rpm");
                    screen1.setCursor(0, 1);
                    screen1.print("Velocidad: " + String(mnEprom.settings.thresholdRpm / 30) + "km/hr");
                    actualizarimagen = millis();
                }
        ...
        case 2:
            if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del meno de opciones
                ...
                ExternalWDT::reset();
                screen1.clear();
                screen1.print(F("2.ALARMA DE"));
                screen1.setCursor(0, 1);
                screen1.print(F("TEMPERATURA"));
                actualizarimagen = millis();
            }

            //SUB MENU 3  LIMITE T    
            //SUB MENU  SUBIR Y BAJAR variable mnEprom.settings.thresholdAlmTemp, guardamos en la EEPROM un numero tipo byte en la posicion 5 como untiliza 1 bites la sigiente variable a escribir la pondremos en 6   
            ...
            while (submenu == 1) {//entramos al submenu solo si se preciona el boton aceptar y estamos fuera de un submeno
                ...
                if (BOTONES() == BOTON_3_ARRIBA) { //si funcion botones retorna 50 es decir que se presion el boton arriba
                    if (millis() > retardo + 400) {
                        mnEprom.settings.thresholdAlmTemp += 1;
                        retardo = millis();
                        Audio::beep();
                    } //condicional para aumento de variable cada 400ms aunque el boton este precionado 
                }

                if (mnEprom.settings.thresholdAlmTemp > 120) {
                    mnEprom.settings.thresholdAlmTemp = 120;
                    Audio::error();
                ...
                if (mnEprom.settings.thresholdAlmTemp < 70) {
                    mnEprom.settings.thresholdAlmTemp = 70;
                    Audio::error();
                }//LIMITES PARA EVITAR PASAR DE CIERTO NUMERO LA VARIABLE

                if (BOTONES() == BOTON_1_ACEPTAR) { // si se presiona aceptar
                    if (millis() > retardo + 400) {
                        EEPROM.put(6, mnEprom.settings.thresholdAlmTemp);
                        retardo = millis();
                        submenu = 0;
                        Audio::guardar();
                    } //añadimos submeno=0 para salir al menu principal despues de guardar
                ...
                if (BOTONES() == BOTON_4_SALIR) { // si se presiona salir
                    if (millis() > retardo + 400) {
                        submenu = 0;
                        retardo = millis();
                        EEPROM.get(6, mnEprom.settings.thresholdAlmTemp);
                        Audio::beep();
                        screen1.clear();
                        screen1.print("SALIENDO");
                        delay(500);
                ...
                if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del menu de opciones
                    ...
                    ExternalWDT::reset();
                    screen1.clear();
                    screen1.setCursor(1, 0);
                    screen1.print("Alarma a: " + String(mnEprom.settings.thresholdAlmTemp) + "C");
                    actualizarimagen = millis();
                }
        ...
        case 3:
            if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del meno de opciones
                ...
                ExternalWDT::reset();
                screen1.clear();
                screen1.print(F("3.CORTE DE"));
                screen1.setCursor(0, 1);
                screen1.print(F("TEMPERATURA"));
                actualizarimagen = millis();
            }
            //SUB MENU 4     
            //SUB MENU  SUBIR Y BAJAR variable mnEprom.settings.thresholdCutTemp, guardamos en la EEPROM un numero tipo byte en la posicion 6 como untiliza 1 bites la sigiente variable a escribir la pondremos en 7   
            ...
            while (submenu == 1) {//entramos al submenu solo si se preciona el boton aceptar y estamos fuera de un submeno
                ...
                if (BOTONES() == BOTON_3_ARRIBA) { //si funcion botones retorna 50 es decir que se presion el boton arriba
                    if (millis() > retardo + 400) {
                        mnEprom.settings.thresholdCutTemp += 1;
                        retardo = millis();
                        Audio::beep();
                    } //condicional para aumento de variable cada 400ms aunque el boton este precionado 
                }

                if (mnEprom.settings.thresholdCutTemp > 120) {
                    mnEprom.settings.thresholdCutTemp = 120;
                    Audio::error();
                }//LIMITES PARA EVITAR PASAR DE CIERTO NUMERO LA VARIABLE
                ...
                if (mnEprom.settings.thresholdCutTemp <= mnEprom.settings.thresholdAlmTemp) {
                    mnEprom.settings.thresholdCutTemp = mnEprom.settings.thresholdAlmTemp + 1;
                    Audio::error();
                    screen1.clear();
                    screen1.print("Temp Corte menor");
                    screen1.setCursor(0, 1);
                    screen1.print("A la de Alarma");
                    delay(2000);
                ...
                if (BOTONES() == BOTON_1_ACEPTAR) { // si se presiona aceptar
                    if (millis() > retardo + 400) {
                        EEPROM.put(7, mnEprom.settings.thresholdCutTemp);
                        retardo = millis();
                        submenu = 0;
                        Audio::guardar();
                    } //añadimos submeno=0 para salir al menu principal despues de guardar
                ...
                if (BOTONES() == BOTON_4_SALIR) { // si se presiona salir
                    if (millis() > retardo + 400) {
                        submenu = 0;
                        retardo = millis();
                        EEPROM.get(7, mnEprom.settings.thresholdCutTemp);
                        Audio::beep();
                        screen1.clear();
                        screen1.print("SALIENDO");
                        delay(500);
                ...
                if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del menu de opciones
                    ...
                    screen1.clear();
                    screen1.setCursor(1, 0);
                    screen1.print("Corte a: " + String(mnEprom.settings.thresholdCutTemp) + "C");
                    actualizarimagen = millis();
                }
            ...
            //OPCION MENU 5 mnEprom.settings.adjustTemp
        case 4:
            if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del meno de opciones
                 
                ExternalWDT::reset();
                screen1.clear();
                screen1.print(F("4.CALIBRAR LA"));
                screen1.setCursor(0, 1);
                screen1.print(F("TEMPERATURA"));
                actualizarimagen = millis();
            }
            //SUB MENU 5     
            //SUB MENU  SUBIR Y BAJAR variable mnEprom.settings.adjustTemp, guardamos en la EEPROM un numero tipo float en la posicion 7 como untiliza 4 bites la sigiente variable a escribir la pondremos en 11   
            ...
            while (submenu == 1) {//entramos al submenu solo si se preciona el boton aceptar y estamos fuera de un submeno
                ...
                if (BOTONES() == BOTON_3_ARRIBA) { //si funcion botones retorna 50 es decir que se presion el boton arriba
                    if (millis() > retardo + 400) {
                        mnEprom.settings.adjustTemp += 0.1;
                        retardo = millis();
                        Audio::beep();
                    } //condicional para aumento de variable cada 400ms aunque el boton este precionado 
                }
                ...
                if (mnEprom.settings.adjustTemp < 0.1) {
                    mnEprom.settings.adjustTemp = 0.1;
                    Audio::error();
                }//Evitamos que la temperatura para el corte sea menor a la temperatura de la alarma e iniciamos apartir de la temperatura para la alarma  
                ...
                if (BOTONES() == BOTON_1_ACEPTAR) { // si se presiona aceptar
                    if (millis() > retardo + 400) {
                        EEPROM.put(8, mnEprom.settings.adjustTemp);
                        retardo = millis();
                        submenu = 0;
                        Audio::guardar();
                    } //añadimos submeno=0 para salir al menu principal despues de guardar
                ...
                if (BOTONES() == BOTON_4_SALIR) { // si se presiona salir
                    if (millis() > retardo + 400) {
                        submenu = 0;
                        retardo = millis();
                        EEPROM.get(8, mnEprom.settings.adjustTemp);
                        Audio::beep();
                        screen1.clear();
                        screen1.print("SALIENDO");
                        delay(500);
                ...
                if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del menu de opciones
                    ...
                    ExternalWDT::reset();
                    temperaturaD2 = ((1023 - analogRead(PIN_ANG_TEMPERATURE)) / mnEprom.settings.adjustTemp);
                    screen1.clear();
                    screen1.print("Temp:" + String(temperaturaD2) + "C ");
                    screen1.setCursor(0, 1);
                    screen1.print(String(mnEprom.settings.adjustTemp));
                    actualizarimagen = millis();
                }
       ...
       //OPCION MENU 6 mnEprom.settings.adjustVoltaje
        case 5://ajuste de la variable ajusteG es un float usado para calibrar la lectura de voltaje de las baterias, escribimos en 15, la siguiente direccion de memoria seria 19
            if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del meno de opciones
                ...
                ExternalWDT::reset();
                screen1.clear();
                screen1.print(F("5.CALIBRAR EL"));
                screen1.setCursor(0, 1);
                screen1.print(F("VOLTAJE"));
                actualizarimagen = millis();
            }
            //SUB MENU 7     
            ...
            while (submenu == 1) {//entramos al submenu solo si se preciona el boton aceptar y estamos fuera de un submeno               
                ...
                if (BOTONES() == BOTON_3_ARRIBA) { //si funcion botones retorna 50 es decir que se presion el boton arriba
                    if (millis() > retardo + 400) {
                        mnEprom.settings.adjustVoltaje += 0.2;
                        retardo = millis();
                        Audio::beep();
                    } //condicional para aumento de variable cada 400ms aunque el boton este precionado 
                }
                ...
                if (mnEprom.settings.adjustVoltaje < 0.2) {
                    mnEprom.settings.adjustVoltaje = 0.2;
                    Audio::error();
                }
                ...
                if (BOTONES() == BOTON_1_ACEPTAR) { // si se presiona aceptar
                    if (millis() > retardo + 400) {
                        EEPROM.put(16, mnEprom.settings.adjustVoltaje);
                        retardo = millis();
                        submenu = 0;
                        Audio::guardar();
                    } //añadimos submeno=0 para salir al menu principal despues de guardar
                ...
                if (BOTONES() == BOTON_4_SALIR) { // si se presiona salir
                    if (millis() > retardo + 400) {
                        submenu = 0;
                        retardo = millis();
                        EEPROM.get(16, mnEprom.settings.adjustVoltaje);
                        Audio::beep();
                        screen1.clear();
                        screen1.print("SALIENDO");
                        delay(500);
                ...
                if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del menu de opciones
                    ...
                    ExternalWDT::reset();
                    bgD = analogRead(PIN_ANG_BATTERY) / mnEprom.settings.adjustVoltaje;
                    screen1.clear();
                    screen1.print("Voltaje:" + String(bgD) + "v ");
                    screen1.setCursor(0, 1);
                    screen1.print(String(mnEprom.settings.adjustVoltaje));
                    actualizarimagen = millis();
                }
        ...
        case 6://ajuste de la variable ajusteG es un float usado para calibrar la lectura de voltaje de las baterias, escribimos en 16, la siguiente direccion de memoria seria 20
            ...
            if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del meno de opciones
                ...
                ExternalWDT::reset();
                screen1.clear();
                screen1.print(F("6.ALARMA DE USO"));
                screen1.setCursor(0, 1);
                screen1.print(F("DE GASOLINA"));
                actualizarimagen = millis();
            }
            //SUB MENU 7     
            ...
            while (submenu == 1) {//entramos al submenu solo si se preciona el boton aceptar y estamos fuera de un submeno               
                ...
                if (BOTONES() == BOTON_3_ARRIBA) { //si funcion botones retorna 50 es decir que se presion el boton arriba
                    if (millis() > retardo + 400) {
                        mnEprom.settings.almBiFuel = true;
                        retardo = millis();
                        Audio::beep();
                    } //condicional para aumento de variable cada 400ms aunque el boton este precionado 
                }
                ...
                if (BOTONES() == BOTON_2_ABAJO) { //si se preciona el boton abajo
                    if (millis() > retardo + 400) {
                        mnEprom.settings.almBiFuel = false;
                        retardo = millis();
                        Audio::beep();
                    }
                }
                ...
                if (BOTONES() == BOTON_1_ACEPTAR) { // si se presiona aceptar
                    if (millis() > retardo + 400) {
                        EEPROM.put(20, mnEprom.settings.almBiFuel);
                        retardo = millis();
                        submenu = 0;
                        Audio::guardar();
                    } //añadimos submeno=0 para salir al menu principal despues de guardar
                ...
                if (BOTONES() == BOTON_4_SALIR) { // si se presiona salir
                    if (millis() > retardo + 400) {
                        submenu = 0;
                        retardo = millis();
                        EEPROM.get(20, mnEprom.settings.almBiFuel);
                        Audio::beep();
                        screen1.clear();
                        screen1.print("SALIENDO");
                        delay(500);
                ...
                if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del menu de opciones
                    ...
                    ExternalWDT::reset();
                    screen1.clear();
                    screen1.print("La Alarma esta:");
                    screen1.setCursor(0, 1);

                    if (mnEprom.settings.almBiFuel) {
                        screen1.print("ACTIVADA");
                    }
                    else {
                        screen1.print("DESACTIVADA");
                    }
        ...
        case 7://Ajuste del tiempo de ahorro de energia
            ...
            if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del meno de opciones
                ...
                ExternalWDT::reset();
                screen1.clear();
                screen1.print(F("7.AHORRO"));
                screen1.setCursor(0, 1);
                screen1.print(F("DE ENERGIA"));
                actualizarimagen = millis();
            }
            //SUB MENU 7     
            ...
            while (submenu == 1) {//entramos al submenu solo si se preciona el boton aceptar y estamos fuera de un submeno               
                ...
                if (BOTONES() == BOTON_3_ARRIBA) {
                    if (millis() > retardo + 400) {
                        mnEprom.settings.thresholdPowerSave += 1;
                        retardo = millis();
                        Audio::beep();
                    } //condicional para aumento de variable cada 400ms aunque el boton este precionado 
                }
                ...
                if (mnEprom.settings.thresholdPowerSave > 255) {
                    mnEprom.settings.thresholdPowerSave = 255;
                    Audio::error();
                }//LIMITES PARA EVITAR PASAR DE CIERTO NUMERO LA VARIABLE
                ...
                if (mnEprom.settings.thresholdPowerSave < 2) {
                    mnEprom.settings.thresholdPowerSave = 2;
                    Audio::error();
                }//LIMITES PARA EVITAR PASAR DE CIERTO NUMERO LA VARIABLE
                ...
                if (BOTONES() == BOTON_1_ACEPTAR) { // si se presiona aceptar
                    if (millis() > retardo + 400) {
                        EEPROM.put(22, mnEprom.settings.thresholdPowerSave);
                        retardo = millis();
                        submenu = 0;
                        Audio::guardar();
                    } //añadimos submeno=0 para salir al menu principal despues de guardar
                ...
                if (BOTONES() == BOTON_4_SALIR) { // si se presiona salir
                    if (millis() > retardo + 400) {
                        submenu = 0;
                        retardo = millis();
                        EEPROM.get(22, mnEprom.settings.thresholdPowerSave);
                        Audio::beep();
                        screen1.clear();
                        screen1.print("SALIENDO");
                        delay(500);
                ...
                if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del menu de opciones
                    ...
                    ExternalWDT::reset();
                    screen1.clear();
                    screen1.print("Esperar para");
                    screen1.setCursor(0, 1);
                    screen1.print("apagado: " + String(mnEprom.settings.thresholdPowerSave * 30) + "min");
        ...
        case 9://ajuste de la variable ajusteG es un float usado para calibrar la lectura de voltaje de las baterias, escribimos en 16, la siguiente direccion de memoria seria 20
            ...
            if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del meno de opciones
                ...
                ExternalWDT::reset();
                screen1.clear();
                screen1.print(F("9.ALARMAS VOLTA"));
                screen1.setCursor(0, 1);
                screen1.print(F("Y ALTERNADOR"));
                actualizarimagen = millis();
            }
            //SUB MENU 7     
            ...
            while (submenu == 1) {//entramos al submenu solo si se preciona el boton aceptar y estamos fuera de un submeno               
                ...
                if (BOTONES() == BOTON_3_ARRIBA) { //si funcion botones retorna 50 es decir que se presion el boton arriba
                    if (millis() > retardo + 400) {
                        mnEprom.settings.almBatery = true;
                        retardo = millis();
                        Audio::beep();
                    } //condicional para aumento de variable cada 400ms aunque el boton este precionado 
                }
                ...
                if (BOTONES() == BOTON_2_ABAJO) { //si se preciona el boton abajo
                    if (millis() > retardo + 400) {
                        mnEprom.settings.almBatery = false;
                        retardo = millis();
                        Audio::beep();
                    }
                }
                ...
                if (BOTONES() == BOTON_1_ACEPTAR) { // si se presiona aceptar
                    if (millis() > retardo + 400) {
                        EEPROM.put(25, mnEprom.settings.almBatery);
                        retardo = millis();
                        submenu = 0;
                        Audio::guardar();
                    } //añadimos submeno=0 para salir al menu principal despues de guardar
                ...
                if (BOTONES() == BOTON_4_SALIR) { // si se presiona salir
                    if (millis() > retardo + 400) {
                        submenu = 0;
                        retardo = millis();
                        EEPROM.get(25, mnEprom.settings.almBatery);
                        Audio::beep();
                        screen1.clear();
                        screen1.print("SALIENDO");
                        delay(500);
                ...
                if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del menu de opciones
                    ...
                    ExternalWDT::reset();
                    screen1.clear();
                    screen1.print("Alarmas estan:");
                    screen1.setCursor(0, 1);

                    if (mnEprom.settings.almBatery) {
                        screen1.print("ACTIVADA");
                    }
                    else {
                        screen1.print("DESACTIVADA");
        ...
        case 10:
            if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del meno de opciones
                ...
                ExternalWDT::reset();
                screen1.clear();
                screen1.print(F("10.RESTABLECER"));
                screen1.setCursor(0, 1);
                screen1.print(F("LOS VALORES"));
                actualizarimagen = millis();
            }
            //SUB MENU 9     
            ...
            while (submenu == 1) {//entramos al submenu solo si se preciona el boton aceptar y estamos fuera de un submeno     
                ...
                if (millis() > actualizarimagen + 500) {//frecuencia con la que se actualiza la imagen del menu de opciones
                    ...
                    ExternalWDT::reset();
                    screen1.clear();
                    screen1.print("precione ACEPTAR");
                    screen1.setCursor(0, 1);
                    screen1.print("para reiniciar");
                    actualizarimagen = millis();
                }
        ...
        case 11: //mostramos la version del software y reiniciamos en automatico pasados 4 segundos
            if (millis() > actualizarimagen + 250) {//frecuencia con la que se actualiza la imagen del meno de opciones
                //  
                screen1.clear();
                screen1.print("11.-soft: " + VERSION_SOFTEARE);
...
bool abreCajaFuerte() {
    ...
    screen1.clear();
    screen1.setCursor(0, 0);screen1.print("Codigo Num:" + String(randNumber));
    //screen1.setCursor(0,1);screen1.print("Pasword: " + String(generaPasswordVariable(randNumber)));
    ...
    while (swich == 0) {
        ...
        if (millis() > tiempoEntreClics + 500) {
            ...
            case BOTON_1_ACEPTAR: //BOTON NUMERO 1
                ...
                passwordIngresado += "1";
                screen1.setCursor(0, 1);screen1.print(passwordIngresado);
    ...
    if (generaPasswordVariable(randNumber) == passwordIngresado.toInt()) {
        ...
        screen1.clear();
        screen1.setCursor(0, 0);screen1.print("Codigo");
        screen1.setCursor(0, 1);screen1.print("Incorrecto");
        Audio::error();
        delay(800);
...
int BOTONES() {
    ...
    int botonera;
    botonera = analogRead(PIN_ANG_BOTONES);
...
void funcionLimpiezaDeRelay() {
    //esta funcion lo que hara es hacer vibrar el relevador de la PIN_RPM para tratar como un ultimo intento
    ...
    // de todas maneras compraremos relevadores de mejor calidad como los de PIN_BOMBA de gasolina rosas de chevy para evitar al maximo este problema
    ...
    screen1.clear();
    screen1.setCursor(0, 0);
    screen1.print("Limpiando los");
    screen1.setCursor(0, 1);
    screen1.print("Contactos  0%");
    Audio::exito();
    ...
    for (int iterador = 1; iterador <= 10; iterador++) {
        ...
        screen1.print("Limpiando los");
        screen1.setCursor(0, 1);
        screen1.print("Contactos  " + String(iterador * 10) + "%");
        ...
        if (iterador <= 5) {
            //aqui activamos la limpieza del relevador de PIN_BOMBA
            contadorlimp = 0;
            ...
            //las siguientes 5 veces del bucle limpiamos el rele de PIN_VENTILADOR
    ...
    screen1.clear();
    screen1.setCursor(0, 0);
    screen1.print("Limpieza ");
    screen1.setCursor(0, 1);
    screen1.print("Finalizada");
    delay(800);
